...
function M.run()
  ...
  assert(value:match("Static combinations:"), "missing static combinations section")
  assert(value:match("Press Command %+ C"), "missing selector preview content")
  assert(value:match("`%.%.%.`: 1 more"), "missing truncated combination count")

  write_result(result_path, {
    ok = true,
    feature = "hover_selector_combinations",
  })
  vim.cmd.qa()
end
