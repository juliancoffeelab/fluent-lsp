...
- asserts there are no diagnostics before any client settings are sent
- sends `workspace/didChangeConfiguration` enabling:
  - `error_on_unsupported_plural_categories`
  - `warn_on_missing_plural_categories`
- waits for four diagnostics on the Latvian file and checks their messages/severities
- opens `workspace/locales/uk/app.ftl`
- attaches the existing client to the new buffer
- checks that Ukrainian reports only a missing `one` category and does not reject `few` or `many`
...
- waits for diagnostics on the English file and checks:
...
- the server publishes diagnostics on open and on `workspace/didChangeConfiguration`
