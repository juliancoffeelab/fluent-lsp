...
function M.run()
  ...
  found = vim.fn.searchpos("commented-download", "n")
  vim.api.nvim_win_set_cursor(0, { found[1], found[2] - 1 })
  found = vim.fn.searchpos("\\V{ $files ->", "nW")
  vim.api.nvim_win_set_cursor(0, { found[1], found[2] - 1 })
  actions = request_code_actions(client_id)
  action = find_action(actions, "Convert selector to prefix form")
  assert(action.edit.changes[change_uri][1].newText == "Descarga { $files } { $files ->\n        [one] archivo.\n        *[other] archivos.\n    }", "unexpected attribute whole->prefix rewrite")
  assert(action.edit.changes[change_uri][1].range.start.line == 12, "attribute rewrite should start at attribute value, not the comment")
  assert(action.edit.changes[change_uri][1].range.start.character == 15, "attribute rewrite should start after the attribute key")
  assert(find_action(actions, "Convert selector to suffix form"), "missing attribute whole->suffix rewrite")

  found = vim.fn.searchpos("install-hint", "n")
  vim.api.nvim_win_set_cursor(0, { found[1], found[2] - 1 })
  found = vim.fn.searchpos("\\$count ->", "nW")
  vim.api.nvim_win_set_cursor(0, { found[1], found[2] - 1 })
  actions = request_code_actions(client_id)
  assert(find_action(actions, "Convert selector to whole form"), "missing local count whole rewrite")
  action = find_action(actions, "Convert selector to suffix form")
  assert(action.edit.changes[change_uri][1].newText == "Copia el enlace de descarga para la cuenta de { $gender ->\n        [female] ella\n        [male] el\n       *[other] elle\n    } en { $count ->\n    [one] { $count } dispositivo ahora.\n    *[other] { $count } dispositivos ahora.\n}", "unexpected local count->suffix rewrite")

  found = vim.fn.searchpos("install-hint", "n")
  vim.api.nvim_win_set_cursor(0, { found[1], found[2] - 1 })
  found = vim.fn.searchpos("\\V{ $gender ->", "nW")
  vim.api.nvim_win_set_cursor(0, { found[1], found[2] - 1 })
  actions = request_code_actions(client_id)
  assert(#actions == 1, "gender selector should only offer whole rewrite")
  action = find_action(actions, "Convert selector to whole form")
  assert(action.edit.changes[change_uri][1].newText == " { $gender ->\n    [female] Copia el enlace de descarga para la cuenta de ella en { $count } { $count ->\n            [one] dispositivo\n           *[other] dispositivos\n        } ahora.\n    [male] Copia el enlace de descarga para la cuenta de el en { $count } { $count ->\n            [one] dispositivo\n           *[other] dispositivos\n        } ahora.\n    *[other] Copia el enlace de descarga para la cuenta de elle en { $count } { $count ->\n            [one] dispositivo\n           *[other] dispositivos\n        } ahora.\n}", "unexpected local gender->whole rewrite")

  found = vim.fn.searchpos("zero-rollout", "n")
  vim.api.nvim_win_set_cursor(0, { found[1], found[2] - 1 })
  found = vim.fn.searchpos("\\$count ->", "nW")
  vim.api.nvim_win_set_cursor(0, { found[1], found[2] - 1 })
  actions = request_code_actions(client_id)
  action = find_action(actions, "Convert selector to whole form")
  assert(action.edit.changes[change_uri][1].newText:match("^ %{% %$count %-%>"), "whole rewrite should preserve = { spacing")
