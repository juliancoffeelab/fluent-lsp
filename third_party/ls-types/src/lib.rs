...
mod tests {
    ...
    #[cfg(feature = "proposed")]
    #[test]
    fn snippet_text_edit_workspace_edit() {
        test_serialization(
            &TextDocumentEdit {
                text_document: OptionalVersionedTextDocumentIdentifier {
                    uri: "file:///test.ftl".parse().unwrap(),
                    version: Some(1),
                },
                edits: vec![OneOf3::Right(SnippetTextEdit {
                    range: Range::new(Position::new(1, 2), Position::new(1, 2)),
                    snippet: "${1:name} $0".to_string(),
                    annotation_id: None,
                })],
            },
            r#"{"textDocument":{"uri":"file:///test.ftl","version":1},"edits":[{"range":{"start":{"line":1,"character":2},"end":{"line":1,"character":2}},"snippet":"${1:name} $0"}]}"#,
        );
    }

    #[cfg(feature = "proposed")]
    #[test]
    fn workspace_edit_client_capabilities_include_snippet_support() {
        test_serialization(
            &WorkspaceEditClientCapabilities {
                snippet_edit_support: Some(true),
                ..WorkspaceEditClientCapabilities::default()
            },
            r#"{"snippetEditSupport":true}"#,
        );
    }
